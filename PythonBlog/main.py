...
# Run it once to create a table with necessary the fields. 
# db.create_all()
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(
            email=form.email.data, password= generate_password_hash(password=form.password.data, method='pbkdf2:sha256', salt_length=8), name=form.name.data
        )
        db.session.add(user)
        db.session.commit()
        login_user(user)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    gravatar = Gravatar(app=app, size=100, rating='x')
